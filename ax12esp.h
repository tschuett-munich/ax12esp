/*
  ax12esp.h - arbotiX Library for AX-12 Servos
...
class AX12
{
  public:
    ...
    static byte ax_rx_buffer[AX12_BUFFER_SIZE];      // buffer de recepciC3n
    static byte ax_rx_Pointer;                       // making these volatile keeps the compiler from optimizing loops of available()
    ...
    int writePosSpeed (int pos, int speed);
    int action ();
  ...
  private:
    ...
    AX12data returnData (byte _srl, byte expectedByteCount);
